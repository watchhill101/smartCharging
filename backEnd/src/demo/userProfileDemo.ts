...
async function createTestUser(): Promise<string> {
  ...
  const testUser = new User({
    ...
    passwordChangedAt: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) // 30天前
  });
...
async function demonstrateUpdateProfile(userId: string) {
  ...
  try {
    ...
    if (result.success) {
      ...
      console.log(`   新昵称: ${result.user?.nickName}`);
      console.log(`   新邮箱: ${result.user?.email}`);
      console.log(`   新地址: ${result.user?.address}`);
    } else {
...
async function demonstrateChangePassword(userId: string) {
  console.log('\n🔐 修改密码...');
...
async function demonstrateUserStatistics(userId: string) {
  console.log('\n📈 获取用户统计信息...');
  ...
  try {
    ...
    if (statistics) {
      console.log('✅ 用户统计信息:');
      console.log(`   资料完整度: ${statistics.profileCompleteness}%`);
      console.log(`   账户年龄: ${statistics.accountAge} 天`);
      console.log(`   最后活跃: ${statistics.lastActiveDate.toLocaleString()}`);
      console.log(`   总登录次数: ${statistics.totalLogins}`);
      console.log(`   安全评分: ${statistics.securityScore}/100`);
    } else {
      console.log('❌ 用户不存在');
    }
  } catch (error) {
...
async function demonstratePhoneUpdate(userId: string) {
  console.log('\n📱 更新手机号...');
  ...
  try {
    ...
    if (result.success) {
      ...
      console.log(`❌ 手机号更新失败: ${result.message}`);
    }
  } catch (error) {
...
async function demonstrateAvatarUpload(userId: string) {
  console.log('\n🖼️ 模拟头像上传...');
  ...
  try {
    ...
    const result = await UserProfileService.uploadAvatar(userId, mockFile);
    ...
    if (result.success) {
      ...
      console.log(`❌ 头像上传失败: ${result.message}`);
    }
  } catch (error) {
...
async function runUserProfileDemo() {
  ...
  try {
    ...
    // 4. 修改密码
    await demonstrateChangePassword(userId);
    ...
    // 7. 获取用户统计信息
    await demonstrateUserStatistics(userId);
    
    // 8. 获取安全信息
    await demonstrateSecurityInfo(userId);
    
    // 9. 更新手机号
    await demonstratePhoneUpdate(userId);
    
    // 10. 模拟头像上传
    await demonstrateAvatarUpload(userId);
